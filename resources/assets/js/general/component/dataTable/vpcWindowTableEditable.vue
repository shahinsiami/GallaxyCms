  ...
  <section>
    ...
    <div v-if="this.showNotify">
      ...
      <div class="vpc_notify_container">
        <div class="vpc_notify">
          <div class="d-flex flex-row w-100 vpc_notify_title">
            {{ this.title }}
          </div>
          <div class="d-flex flex-row w-100 vpc_notify_error">
            {{ this.message }}
          </div>
          <div
    ...
    <div v-if="tableInfo == ''" class="vpc_loader">
      <div class="vpc_loader_img">
        <img
          width="300"
          height="300"
          src="photo/module/Panel/forms/loader.gif"
        />
      </div>
    </div>
    <!--  -->
    <div v-if="tableInfo !== ''">
      <div class="row vpc_table_header p-3" v-if="this.tableInfo.data !== ''">
        ...
        <!--  -->
        <div
          class="my-2 col-12 col-lg-6 d-flex flex-row justify-content-center align-items-center"
        >
          <div class="vpc_search_title mx-1">جستجو بر اساس ستون</div>
          <div v-for="(searchItem, index) in tableColumn" :key="index">
            <div
              v-bind:class="[
                index === 0 ? 'vpc_search_title_object_active' : '',
              ]"
    ...
    <div class="p-3">
      <div
        class="d-flex flex-row my-2 align-items-center vpc_datatable_date w-100"
      >
      ...
      >
        ...
        >
          ...
          <span v-if="columnHeader.headerType === 'headerOrder'">
            <div
              class="d-flex flex-row justify-content-center align-items-center flex-nowrap"
            >
              <div>
                <img
          ...
          <!---->
          <span v-if="columnHeader.headerType === 'headerTitle'">{{
            columnHeader.headerTitle
          }}</span>
          <!---->
          <span v-if="columnHeader.headerType === 'headerOperation'"
            >عملیات</span
          >
          <!---->
        </div>
      ...
      <div v-if="this.tableColumn !== ''">
        ...
        >
          ...
          >
            <!-- number -->
            <p class="h-100" v-if="columnItem.type === 'number'">
              {{ rowIndex + 1 }}
            </p>
            <!-- number -->
            <!-- date -->
            ...
            <!-- date -->
            <!-- string -->
            <p v-if="columnItem.type === 'string'">
              {{ item[columnItem.title] }}
            </p>
            <!-- string -->
            <!-- substring -->
            <p v-if="columnItem.type === 'substring'">
              <span v-if="item[columnItem.title].lenght > 1"
            ...
            <!-- image -->
            <!-- relation -->
            <p v-if="columnItem.type === 'relation'">
              {{ item[columnItem.title][columnItem.relation] }}
            </p>
            <!-- relation -->
            <!-- manyRelation -->
            ...
            <p v-if="columnItem.type === 'manyRelationObjectWithTwoTitle'">
              ...
              >
                {{
                  manyrelation[columnItem.relationTitle][
                    columnItem.relationSubTitleOne
                  ]
                }}
                <span v-if="columnItem.relationSubTitleTwo">{{
                  manyrelation[columnItem.relationTitle][
                    columnItem.relationSubTitleTwo
                  ]
                }}</span>
              </span>
            </p>
            ...
            <p v-if="columnItem.type === 'manyRelationSingleObject'">
              {{
                item[columnItem.title][columnItem.relationTitle][
                  columnItem.relation
                ]
              }}
            </p>
            ...
            <!-- input -->
            <div
              v-if="columnItem.type === 'input'"
              class="d-flex flex-row w-100 h-100 align-items-center justify-content-center position-relative"
            >
              <input
                :id="columnItem.title + item.id"
                :value="item[columnItem.title]"
                class="vpc_table_input w-100 h-100"
              />
              <img
            ...
            <!-- select -->
            <div
              v-if="columnItem.type === 'selectbox'"
            ...
            >
              ...
              >
                <option
                  :selected="item[columnItem.title] == selectOption.id"
                  v-for="(selectOption,
                  keySelectOption) in columnItem.selectBox"
                  :key="keySelectOption"
            ...
            <p v-if="columnItem.type === 'operationEditColumn'">
              <span>
                <img
                  class="img-fluid"
                  src="photo/module/Panel/forms/accept.png"
      ...
      >
        ...
        >
          <div v-for="(item, index) in this.tableInfo.last_page" :key="index">
            <div
              v-bind:class="[
                index === 0 ? 'vpc_table_paginate_object_active' : '',
              ]"
...
export default {
  ...
  methods: {
    ...
    changeStartDate(e) {
      ...
      if (this.selectedId) {
        this.$store.dispatch(this.getData, [this.data, this.selectedId]);
      } else {
    ...
    editColumnSubmit(data) {
      const form = new FormData();
      form.append(this.firstColumnSearch, data[this.firstColumnSearch]);
      this.editData.forEach((item) => {
        form.append(item, document.getElementById(item + data.id).value);
      });
      ...
      axios
        ...
        .then((response) => {
          ...
          this.title = "تایید";
          this.message = "این ردیف اصلاح شد";
          this.type = "success";
          setTimeout(() => {
