<template>
  <div class="content">
    <div class="row p-2">
      <div class="col-6 d-flex flex-row justify-content-center align-items-center"></div>
      <div class="col-6">
        <windowHeader :windowTitle="this.window.title" :windowTitleLine="this.window.titleLine"></windowHeader>
      </div>
    </div>
    <windowTableWithoutExcel
      :selectStore="this.table.selectStore"
      :getData="this.table.getData"
      :tableColumn="this.table.column"
      :fileName="this.table.fileNmae"
      :selectRow="this.table.selectRow"
      :openSelectRow="this.table.openSelectRow"
      :deleteRow="this.table.deleteRow"
      firstColumnSearch="id"
    ></windowTableWithoutExcel>
  </div>
</template>
<style scoped>
</style>

<script>
export default {
  name: "cropDetailTable",
  data() {
    return {
      window: {
        headerid: "cropDetailTable",
        title: "خصوصیات محصولات",
        titleLine: "محصولات/خصوصیات/خصوصیات/خصوصیات محصولات",
      },
      table: {
        selectStore: "formCropDetail",
        getData: "cropDetailTable",
        openSelectRow: ["panel", "cropDetailEdit", "ویرایش جزییات محصولات"],
        selectRow: "selectCropDetail",
        deleteRow: "/api/v1/deleteCropDetail/",
        excel: "/api/v1/excelDetail",
        fileNmae: "detail.xls",
        column: [
          {
            title: "id",
            type: "number",
            headerType: "headerId",
            col: "col-2",
            headerType: "headerOrder",
            headerTitle: "شماره",
          },
          {
            title: "name",
            type: "string",
            headerType: "headerName",
            col: "col-2",
            headerType: "headerOrder",
            headerTitle: "نام",
          },
          {
            title: "description",
            type: "string",
            headerType: "headerTitle",
            headerTitle: "توضیحات",
            col: "col-6",
          },
          {
            title: "operation",
            type: "operatoin",
            headerType: "headerOperation",
            col: "col-2",
          },
        ],
      },
    };
  },
};
</script>
