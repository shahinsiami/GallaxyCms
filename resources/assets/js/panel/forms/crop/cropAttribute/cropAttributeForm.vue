    ...
    <div style="direction:rtl" v-if="this.refreshPage" class="container-fluid">
      ...
      <div class="row" v-bind:class="[this.tab1 ? '' : 'vpc_nav_item_selected_form']">
        <listSelector
          selectedItem="name"
          listName="محصول"
          listSource="allCroptForAttribute"
          :getSource="this.$store.state.formCropAttribute.allCroptForAttribute"
          :lastValue="data.crop_id"
          @selectid=" data.crop_id = $event.id;crop_sub_segment_id = $event.crop_sub_segment_id "
        ></listSelector>
      </div>
      <!---->
      <div class="row" v-bind:class="[this.tab2 ? '' : 'vpc_nav_item_selected_form']">
        <keyValueInput
          ...
          source="cropDetailForCropsAttribute"
          :getSource="this.$store.state.formCropAttribute.cropDetailForCropsAttribute"
...
export default {
  ...
  data() {
    return {
      ...
      refreshPage: true,
      window: {
        headerid: "cropAttributeForm",
        title: "ثبت جزئیات محصول",
        titleLine: "محصولات/جزئیات / ثبت جزئیات محصول",
      },
      submit: {
        buttonTitle: "ثبت",
        axiosUrl: "/api/v1/registerCropAttribute",
      },
      crop_sub_segment_id: "",
      data: {
        crop_id: "",
