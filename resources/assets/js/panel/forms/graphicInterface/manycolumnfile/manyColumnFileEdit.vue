...
export default {
  ...
  data() {
    return {
      refreshPage: true,
      window: {
        headerid: "manyColumnFileEdit",
        title: "ویرایش چندستونه فایلی",
        titleLine: "ماژول/ماژول های گرافیکی/چندستونه فایلی/ویرایش چندستونه فایلی",
      },
      edit: {
        buttonTitle: "ویرایش",
        axiosUrl: "/api/v1/editManyColumnFile/",
        response: "selectManyColumnFile",
      },
      statusOption: [
        { id: 0, name: "غیر فعال" },
        { id: 1, name: "فعال" },
  ...
  computed: {
    lastId: {
      get() {
        if (this.$store.state.ComponentManyColumnFile.selectManyColumnFile.id) {
          return this.$store.state.ComponentManyColumnFile.selectManyColumnFile
            .id;
        } else {
          return "";
        }
